...
function generateToken(user, tokenType = 'access') {
  ...
  const secret = isAccessToken ? JWT_CONFIG.ACCESS_TOKEN_SECRET : JWT_CONFIG.REFRESH_TOKEN_SECRET;
...
apiRouter.post('/user', (req, res) => {
  try {
    ...
    // 1. 필수 입력사항 체크
    if (!user || !user.username || !user.email || !user.password || !user.accountname) {
      return res.status(400).json({
    ...
    // 6. accountname 중복 체크
    const existingAccountname = db.get('users').find({ accountname: user.accountname }).value();
...
apiRouter.put('/user', (req, res) => {
  try {
    ...
    const updatedUserData = {
      ...currentUser,
      username: user.username !== undefined ? user.username : currentUser.username,
      accountname: user.accountname !== undefined ? user.accountname : currentUser.accountname,
...
apiRouter.get('/profile/:accountname', (req, res) => {
  try {
    ...
    const db = router.db;
    const targetUser = db.get('users').find({ accountname: accountname }).value();
...
apiRouter.post('/profile/:accountname/follow', (req, res) => {
  try {
    ...
    db.get('users').find({ _id: targetUser._id }).assign({ follower: targetUserFollower }).write();

    // 11. 업데이트된 대상 사용자 정보 다시 조회
    const updatedTargetUser = db.get('users').find({ _id: targetUser._id }).value();
...
apiRouter.delete('/profile/:accountname/unfollow', (req, res) => {
  try {
    ...
    // 6. 언팔로우 처리 - 배열에서 제거
    const updatedCurrentUserFollowing = currentUserFollowing.filter((id) => id !== targetUser._id);
    const updatedTargetUserFollower = targetUserFollower.filter((id) => id !== currentUser._id);
...
apiRouter.get('/user/searchuser', (req, res) => {
  try {
    ...
    const filteredUsers = allUsers.filter((user) => {
      ...
      return username.includes(searchKeyword) || accountname.includes(searchKeyword);
    });
...
apiRouter.get('/post/feed', (req, res) => {
  try {
    ...
    const followingPosts = allPosts.filter((post) => followingIds.includes(post.authorId));
...
apiRouter.get('/post/:accountname/userpost', (req, res) => {
  try {
    ...
    // 5. 해당 사용자의 게시글만 필터링
    const userPosts = allPosts.filter((post) => post.authorId === targetUser._id);
...
apiRouter.post('/post/:post_id/heart', (req, res) => {
  try {
    ...
    // 5. 이미 좋아요한 게시글인지 확인
    const existingHeart = db.get('hearts').find({ postId: post_id, userId: decoded._id }).value();
...
apiRouter.get('/post/:post_id/comments', (req, res) => {
  try {
    ...
    const postComments = allComments.filter((comment) => comment.postId === post_id);
...
apiRouter.get('/product/:accountname', (req, res) => {
  try {
    ...
    // 5. 해당 사용자의 상품만 필터링
    const userProducts = allProducts.filter((product) => product.authorId === targetUser._id);
...
apiRouter.put('/product/:product_id', (req, res) => {
  try {
    ...
    // 7. 상품 업데이트
    const updatedData = {
      itemName: product.itemName !== undefined ? product.itemName : existingProduct.itemName,
      ...
      itemImage: product.itemImage !== undefined ? product.itemImage : existingProduct.itemImage,
    };
    ...
    // 9. 작성자 정보 조회
    const author = db.get('users').find({ _id: updatedProduct.authorId }).value();
